...
public partial class Pause : Node2D
{
    ...
    public override void _Ready()
    {
        ...
        items = GD.Load<PackedScene>("res://Assets/Scenes/SingleScenes/Windows/Items.tscn")
            .Instantiate<Node2D>();
        stats = GD.Load<PackedScene>("res://Assets/Scenes/SingleScenes/Windows/Stats.tscn")
            .Instantiate<Stats>();
    ...
    public override void _PhysicsProcess(double delta)
    {
        if (Visible)
        {
            ...
            var newposnode = currNode.GetNode<Marker2D>(
                "pos-" + currNode.Col.ToString() + "-" + currNode.Row.ToString()
            );
            ...
            soulIn.GlobalPosition = soulIn.GlobalPosition.Lerp(
                posnode.GlobalPosition,
                (float)(delta * Speed)
            );
